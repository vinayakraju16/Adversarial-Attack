...
$RequiredRelativeFiles = @(
    ...
    "scripts/run_pipeline_encoder_gpu.sh"
)
...
# ----------------------------------------------------------------------------
# PHASE 2 - RUN ENCODER PIPELINE ON GPU
# ----------------------------------------------------------------------------

Write-Host "[INFO] Phase 2/3: Running encoder pipeline on GPU ..."
ssh $RemoteHost "bash '$RemoteRepoPath/scripts/run_pipeline_encoder_gpu.sh'"
if ($LASTEXITCODE -ne 0) { Fail-Step "Encoder GPU pipeline failed. Check the SSH output above for the error." }
...
Write-Host ""
Write-Host "[DONE] Encoder remote pipeline finished."
